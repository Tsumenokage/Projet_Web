...
$checkUser = $db->prepare("SELECT * FROM utilisateurs WHERE IdUtilisateur = :id AND Login = :login ");
...
$query = $db->prepare("
	SELECT message_id, message_user, message_time, message_text, IdUtilisateur, Login
	FROM chat_messages
	LEFT JOIN utilisateurs ON utilisateurs.IdUtilisateur = chat_messages.message_user
...
if($count != 0) {
	...
	while ($data = $query->fetch()) {
		...
		if($prev != $data['IdUtilisateur']) {
			// contenu du message
			$text .= '<a href="#post" onclick="insertLogin(\''.addslashes($data['Login']).'\')" style="color:black">';			
			$text .= '&nbsp;<span style="color:'.$color.'">'.$data['Login'].'</span>';
			$text .= '</a>';	
		}
		$text .= '</td>';			
