...
function getMessages() {
	...
	$.getJSON('module/phpscripts/get-message.php?dateConnexion='+$("#dateConnexion").val(), function(data) {
...
function postMessage() {
	...
	$.ajax({
		type: "POST",
		url: "module/phpscripts/post-message.php",
...
$(document).ready(function() {
	...
	if(document.getElementById('message')) {
		//Actualise les membres connectés
	...
	$.getJSON('module/phpscripts/get-online.php', function(data) {
		...
		if(data['error'] == '0') {		
			...
			for (var id in data['list']) {
				...
				online += '<img src="module/status-'+image+'.png" /> ';
...
function setStatus(status) {
	...
	$.ajax({
		type: "POST",
		url: "module/phpscripts/set-status.php",
